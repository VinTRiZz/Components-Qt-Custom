...
namespace Qt_Custom::DrawWidget
{
...
struct PaintConfig
{
    ...
    void drawFigure(const QVector<Point> & points, const bool isFilled)
    {
        if (isFilled)
        {
...
DrawViewWidget::~DrawViewWidget()
{
    for (auto f : d->figures)
        delete f;
}

void DrawViewWidget::addFigure(AbstractFigure *f) { d->figures.push_back(f); }
void DrawViewWidget::removeFigure(AbstractFigure *f) { d->figures.removeOne(f); }
void DrawViewWidget::removeFigure(int figureIndex) { if (figureIndex < d->figures.size()) d->figures.removeAt(figureIndex); }
...
void DrawViewWidget::mousePressEvent(QMouseEvent *e)
{
    while (d->drawLock);
    d->drawLock = true;
    if (e->button() == Qt::RightButton)
    {
        ...
        d->pClickFigure->addPoint(Point(e->x(), e->y()));
    d->drawLock = false;
    repaint();
}

void DrawViewWidget::drawData()
{
